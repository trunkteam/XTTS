...
class AudioPairGANDataset(Dataset):
    """
    ...
    NOTE: Dataset example [["original_wav_path_1", "mel_wav_path1"], ["original_wav_path_2", "mel_wav_path2"], ["original_wav_path_3", "mel_wav_path3"],]
    """
    def __init__(
        self,
        ap,
        ...
        self.compute_feat = compute_feat
    ...
    def load_item(self, idx):
        ...
        if self.compute_feat:
            ...
            original_wav_path, mel_wav_path = self.item_list[idx]
            # print(wavpath)

            if self.use_cache and self.cache[idx] is not None:
                audio, mel = self.cache[idx]
            else:
                audio = self.ap.load_wav(mel_wav_path)
                mel = self.ap.melspectrogram(audio)
                audio = self.ap.load_wav(original_wav_path)
